...
#' @param metadata_cols A character vector of column names expected in metadata.
#'   If NULL (default), all metadata columns are extracted.
#' @param out_dir Character. Directory where the files will be saved.
#' @param base_name Character. Prefix for the saved filenames.
#' @param assay Character. The specific assay to get data from or set data for; defaults to
#'  \code{\link[SeuratObject]{DefaultAssay}}
#' @param layer The name of the layer to get (defaults to "count" for raw counts).
#'
#' @return Logical TRUE if successful.
#' @export
save_seurat_components <- function(seurat_obj,
                                   metadata_cols = NULL,
                                   out_dir = getwd(),
                                   base_name = "data",
  ...
  if (is.null(metadata_cols)) {
    ...
    cli::cli_alert_info("No metadata columns specified. Extracting all ({length(metadata_cols)}) metadata columns.")

  } else {

    missing_cols <- setdiff(metadata_cols, colnames(seurat_obj@meta.data))

    if (length(missing_cols) > 0) cli::cli_abort("The following columns are missing from metadata: {missing_cols}")
  }

  counts_matrix <- SeuratObject::GetAssayData(seurat_obj, assay = assay, layer  = layer)

  if (!inherits(counts_matrix, "dgCMatrix")) counts_matrix <- as(counts_matrix, "dgCMatrix")


  metadata_df <- seurat_obj@meta.data[, metadata_cols, drop = FALSE]

  metadata_df <- metadata_df[colnames(counts_matrix), , drop = FALSE]

  if (!all(rownames(metadata_df) == colnames(counts_matrix))) cli::cli_abort("Failed to align metadata rownames with counts matrix colnames.")

  if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

  counts_path <- nf(filename = paste0(base_name, "_counts.rds"), filepath = out_dir)
  meta_path   <- nf(filename = paste0(base_name, "_metadata.rds"), filepath = out_dir)

  saveRDS(counts_matrix, file = counts_path)
  saveRDS(metadata_df, file = meta_path)

  message("Files saved successfully to: ", out_dir)
  return(TRUE)
}
