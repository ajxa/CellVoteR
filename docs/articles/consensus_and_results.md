...
After running the ensemble annotation methods, the next step is to
resolve the results into a final set of cell labels. This is done
through a configurable consensus resolution step. This step is
intentionally separate from the annotation methods, allowing you to
adjust voting parameters and run
[`resolve_consensus_labels()`](https://ajxa.github.io/CellVoteR/reference/resolve_consensus_labels.md)
without needing to re-running the entire annotation pipeline.
...
The process of resolving a final label for each cell is based on a
simple voting scheme. Each of the four primary annotation workflows
casts a “vote” for a label, and the two global tie-breakers are then
