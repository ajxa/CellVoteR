...
to construct one from your raw data.
...
[`create_sce()`](https://ajxa.github.io/CellVoteR/reference/create_sce.md)
also accepts file paths, which is useful for large datasets where the
...
function co-ordinates the key preprocessing step. This function
performs/applies, the following calculations and logic:
...
orchestrates all of the individual annotation pipelines and returns a
...
After running
[`run_cellvoter()`](https://ajxa.github.io/CellVoteR/reference/run_cellvoter.md),
the returned SingleCellExeriment object is populated with all of th
intermediate cluster labels, which can be accessed by querying the
colData() columns of the object:

| Column | Description |
|----|----|
| **cluster_broad_hvg** | Pre-existing HVG clusters from [`prepare_sce()`](https://ajxa.github.io/CellVoteR/reference/prepare_sce.md) |
| **cluster_broad_reduced** | Pre-existing reduced clusters from [`prepare_sce()`](https://ajxa.github.io/CellVoteR/reference/prepare_sce.md) |
...
> [`rank_cluster_markers()`](https://ajxa.github.io/CellVoteR/reference/rank_cluster_markers.md)
> but at different pipeline stages. *broad_args* affects how broad
...
If
[`prepare_sce()`](https://ajxa.github.io/CellVoteR/reference/prepare_sce.md)
warns about low fine marker overlap, inspect which labels are most
affected:
