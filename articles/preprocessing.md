...
After creating the `SingleCellExperiment`, the
[`assess_cell_quality()`](https://ajxa.github.io/CellVoteR/reference/assess_cell_quality.md)
helper function can be used to calculate per-cell QC metrics and
optionally remove low-quality cells before downstream analysis. This is
usually the first step after creating the SCE, as it ensures that
downstream processes are not affected by low-quality cells that may have
aberrant expression profiles.
...
helper function, which takes the raw SCE and returns a new SCE with
normalised logcounts.
...
The final step before annotation is to build the analysis tracks - the
feature spaces and cluster structures on which the annotation methods
depend. This is done by the
[`prepare_sce()`](https://ajxa.github.io/CellVoteR/reference/prepare_sce.md)
...
Internally, the
[`prepare_sce()`](https://ajxa.github.io/CellVoteR/reference/prepare_sce.md)
function applies a series of processing steps:
...
During the this final preparation step, key parameters for PCA and
clustering are estimated automatically based on the number of cells in
the dataset, using bounded $`log/sqrt`$ scaling via the
...
[`prepare_sce()`](https://ajxa.github.io/CellVoteR/reference/prepare_sce.md)
checks the supplied marker lists against the expression matrix and
reports on the degree of overlap. If a large proportion of fine markers
are missing from the dataset, this may indicate an issue with gene
naming conventions or platform-specific gene capture, which could affect
annotation performance. The function captures this information and
details the labels that are most affected, which can be inspected as
follows:
...
annotation. The next step is to run the ensemble annotation methods
using
[`run_cellvoter()`](https://ajxa.github.io/CellVoteR/reference/run_cellvoter.md),
which will generate per-method and consensus labels for each cell based
on the supplied markers and cluster structures. For more details on the
annotation methods, see the [Annotation
